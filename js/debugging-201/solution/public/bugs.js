angular.module('DaBug', ['ngRoute'])
    // BUG: misspell controller  
    // MESSAGE: Dev Console Message: bugs.js:X Uncaught TypeError: angular.module(...).contoller is not a function
    // FIX: Fix spelling of controller
    //.contoller('BugCtrl', bugControl)       
    //.contoller('BugProfile', bProfile)
    .controller('BugProfile', bProfile)
    ...
    bCtrl.getBugs = function () {
        bugFactory.getBugs()
            .then(function (res) {
                ...
                // MESSAGE: bugs.js:39 Uncaught SyntaxError: missing ) after argument list
                // FIX: insert the missing )
                // };
            });
    };
